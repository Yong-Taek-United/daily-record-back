...
export class UsersService {
  ...
  async create(userData: CreateUserDto) {
    ...
    if (email.length < 5) throw new BadRequestException(['email', '이메일이 너무 짧습니다.']);

    if (isExist) throw new ConflictException(['email', '이미 존재하는 이메일입니다.']);

    if (username.length < 2) throw new BadRequestException(['username', '2자 이상의 이름을 입력해주십시오.']);

    if (password !== password2) throw new BadRequestException(['password2', '비밀번호를 다시 확인해주십시오.']);
  ...
  async update(id: number, userData: UpdateUserDto) {
    if (userData.password) {
      if (userData.password.length < 8) throw new BadRequestException('8자 이상의 비밀번호를 입력해주십시오.');
